...
batchEffect <- args[3]
samplesheet <- args[4]
dnaSamplesheet <- args[5]
donorSamplesheet <- args[6]
...
if (condition != "ALL"){
  covariates <- covariates %>% filter(Condition == condition) %>%
    arrange(match(Donor, colnames(CPMadjTMM_invNorm)))
} else {
  
  # Match covariates with sample colnames of normalized counts
  covariates <- covariates %>%
    arrange(match(Sample, colnames(CPMadjTMM_invNorm)))
}


# Correct for batch effects with limma
CPMadjTMM_invNorm_batchCorrected <- 
  removeBatchEffect(CPMadjTMM_invNorm, batch = covariates %>% pull(batchEffect))


# Calculate variances to remove columns where variance is 0 (can't do PCA)
variances <- CPMadjTMM_invNorm_batchCorrected %>% 
  t() %>% apply(2, var, na.rm = TRUE)

# Get PC's
if (length(which(variances == 0) > 0)){
  
  CPMadjTMM_invNorm_pca <- CPMadjTMM_invNorm_batchCorrected %>%
...
write_csv(var_explained, 
          file = paste0("output/pca/", condition, "_PC_", batchEffect, "_corrected_varExplained.csv"))
...
# Write to file
write_csv(final_pca, 
          file = paste0("output/pca/", condition, "_PCs_", batchEffect, "_corrected.csv"))